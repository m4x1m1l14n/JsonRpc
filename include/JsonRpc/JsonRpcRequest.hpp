...
namespace JsonRpc
{
	...
	{
	public:
		JsonRpcRequest(uint64_t id, const std::string& method, const json11::Json& params)
			: JsonRpcMessage(id)
			, m_method(method)
			, m_params(params)
		{
		...
		static JsonRpcRequest Parse(const json11::Json& json)
		{
			...
			(
				json["id"].uint64_value(),
				json["method"].string_value(),
				json["params"]
			);
		}

		static JsonRpcRequest Create(uint64_t id, const std::string& method, const json11::Json& params)
		{
